...
- files: three to ten paths, each under app/, lib/ or tests/, each with a one-line purpose. Always include app/page.tsx, one route file under app/api/, and at least one test file. Place test files under tests/api/ (api and lib tests) or tests/ui/ (page tests), never directly in tests/, so each test file has exactly one owner when two builders work in parallel.
